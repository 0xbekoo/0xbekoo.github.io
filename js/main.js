(() => {
  ...
  (function() {
    ...
    const setDarkMode = (isDark) => {
      ...
      const iconHtml = `<a id="nav-${isDark ? "sun" : "moon"}-btn" class="nav-icon dark-mode-btn"></a>`;
      document.getElementById("sub-nav")?.insertAdjacentHTML("beforeend", iconHtml);
  ...
  window.safeImport = async function(url, integrity) {
    if (!integrity) {
      return import(url);
    }
